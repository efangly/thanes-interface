...
exports.insertMiddle = async (req, res) => {
  ...
  try{
    let frequency = await Frequency.findAll()
    let ward = await Ward.findAll()
    let { data, status } = validateorder(order, frequency, ward)
    if(status){
      await Middle.bulkCreate(data).then((middle) => {
